...
//        uint collatAssetPrice = getCollatAssetPrice('Sonic')/getCollatAssetPrice('ETH');
        require(collateralBalance[_borrower] * getCollatAssetPrice(sonicAggregatorAddress) / getCollatAssetPrice(ethAggregatorAddress) <= calculateLiquidationPoint(_borrower), "Position can't be liquidated!");
        _;
    }
...
//        uint collatAssetPrice = getCollatAssetPrice('Sonic')/getCollatAssetPrice('ETH');
        limit = (collateralBalance[_borrower] * getCollatAssetPrice(ethAggregatorAddress) * 70 / 100 / getCollatAssetPrice(sonicAggregatorAddress)) - borrowBalance[_borrower];
    }
...
//        baseAsset.approve(address (this), _amount);
    ...
    function withdrawLend(uint _amount) public {
        ...
        if (baseAsset.balanceOf(address(this)) >= withdrawAmount) {
            ...
            withdrawAmount = withdrawAmount * getCollatAssetPrice(ethAggregatorAddress) / getCollatAssetPrice(sonicAggregatorAddress);
